...
# create factor column and relabel each stage by #
iTRAQ_key <- transform(iTRAQ_key, id=as.numeric(factor("AJCC Stage")))

#iTRAQ_key$Subtype <- as.numeric(factor(iTRAQ_key$"AJCC Stage", levels=unique(iTRAQ_key$"AJCC Stage")))
...
iTRAQ_matrix <- t(iTRAQ_matrix)
    ...
    #Optimal Parameters used below
    Stage_Classifier <- svm(kNN_common_matrix,numbers_key_factor,type="C-classification",
                            kernel="radial",gamma=1E-6,cost=100)

    #run preidction svm on iTRAQ data, trained on KNN data
    predictions_iTRAQ <- predict(Stage_Classifier, iTRAQ_matrix)

    #Correct predictions on testing data (iTRAQ)
    i<-0
    correct_iTRAQ <- 0
    
    Stage1Count <- 0 #number of stage 1's in iTRAQ
    ...
    Pred1Count <- 0 #number of times classifier predicted Stage 1
    Pred2Count <- 0
    Pred3Count <- 0
    Pred4Count <- 0
